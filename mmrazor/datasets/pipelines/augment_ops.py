...
"""Random augmentation implemented by https://github.com/rwightman/pytorch-
image-models/blob/master/timm/data/auto_augment.py."""
import math
import random
import re
import warnings
...
def distort(img, v, **__):
    ...
    return img.transform(
        img.size, PIL.Image.MESH, generated_mesh, resample=PIL.Image.BICUBIC)


def zoom(img, v, **__):
    # assert 0.1 <= v <= 2
    w, h = img.size
    image_zoomed = img.resize(
        (int(round(img.size[0] * v)), int(round(img.size[1] * v))),
...
def erase(img, v, **__):
    ...
    if len(img.getbands()) == 1:
        rectangle = PIL.Image.fromarray(
            np.uint8(np.random.rand(w_occlusion, h_occlusion) * 255))
    else:
        rectangle = PIL.Image.fromarray(
            np.uint8(
                np.random.rand(w_occlusion, h_occlusion, len(img.getbands())) *
                255))
...
class RandAugmentTransform(object):

    def __init__(self, config_str, hparams):
        self.config_str = config_str
        self.hparams = hparams
        self.magnitude = _MAX_LEVEL  # default to _MAX_LEVEL for magnitude (
        # currently 10)
        self.num_layers = 2  # default to 2 ops per image
    ...
    def __call__(self, results):
        ra_ops = rand_augment_ops(
            magnitude=self.magnitude, hparams=self.hparams)
        for key in results.get('img_fields', ['img']):
            img_np = results[key]
            img = Image.fromarray(img_np)
            res_aug = RandAugment(
                ra_ops, self.num_layers, choice_weights=self.choice_weights)(
                    img)
            results[key] = np.asarray(res_aug).astype(img_np.dtype)
        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(config_str={self.config_str}, '
        repr_str += f'hparams={self.hparams})'
        return repr_str


def rand_augment_transform(config_str, hparams):
    magnitude = _MAX_LEVEL  # default to _MAX_LEVEL for magnitude (currently
    # 10)
...
class AutoAugmentOp:
    ...
    def __init__(self, name, prob=0.5, magnitude=10, hparams=None):
        ...
        if 'interpolation' in hparams:
            if hparams['interpolation'] == 'bicubic':
                resample = Image.BICUBIC
            else:
                resample = Image.BILINEAR
        else:
            resample = _RANDOM_INTERPOLATION
...
def _get_pixels(per_pixel,
                ...
                dtype=torch.float32,
                device='cuda'):
    # NOTE I've seen CUDA illegal memory access errors being caused by the
    # normal_() paths, flip the order so normal is run on CPU if this
    # becomes a problem Issue has been fixed in master
    # https://github.com/pytorch/pytorch/issues/19508
    if per_pixel:
...
class RandomResizedCropAndInterpolation:
    ...
    def get_params(img, scale, ratio):
        ...
        Args: img (PIL Image): Image to be cropped. scale (tuple): range of
        size of the origin size cropped ratio (tuple): range of aspect ratio
        of the origin aspect ratio cropped
...
class RandomErasing:
    ...
    'Random Erasing Data Augmentation' by Zhong et al. See
    ...
    mean and std. Args: probability: Probability that the Random Erasing
    operation will be performed. min_area: Minimum percentage of erased
    area wrt input image area. max_area: Maximum percentage of erased
    area wrt input image area. min_aspect: Minimum aspect ratio of
    erased area. mode: pixel color mode, one of 'const', 'rand',
    or 'pixel' 'const' - erase block is constant color of 0 for all
    channels 'rand'  - erase block is same per-channel random (normal)
    color 'pixel' - erase block is per-pixel random (normal) color
    max_count: maximum number of erasing blocks per image, area per box
    is scaled by count. per-image count is randomly chosen between 1 and
    this value.
    """
    ...
    def __call__(self, input):
        if len(input.size()) == 3:
            ...
            # skip first slice of batch if num_splits is set (for clean
            # portion of samples)
            batch_start = batch_size // self.num_splits if \
                self.num_splits > 1 else 0
